 ...
 * @Last Modified time: 2020-03-17 10:36:50
 -->
<template>
	<view class="page home-page">
		...
		<view class="header">
			...
			>
				<swiper-item v-for="(v,k) in swiperItems" :key="k">
					<view class="banner-item">
						<img class="banner-image" mode="widthFix" :src="v.image" />
					</view>
				</swiper-item>
			</swiper>
			<zBarrage
		...
		<view class="tabs">
			<view @click="switchTab('new')" class="tab" :class="{'tab_on':isNew}">
				新品
				<view class="tab-icon">◆ NEW ◆</view>
			</view>
			<text class="tab-separate"></text>
			<view @click="switchTab('all')" class="tab tab_all" :class="{'tab_on':!isNew}">
				全部
				<view class="tab-icon">◆ ALL ◆</view>
			</view>
		</view>
		<view class="items">
			<view class="items-box" v-show="isNew">
				<view class="item" v-for="(v,k) in itemsNewData" :key="k" @click="goInfo(v,k)">
					<view class="item-tag">
						<image src="/static/top-icon.png" mode="aspectFit" class="item-icon" v-if="v.isTop" />
						<image src="/static/new-icon.png" mode="aspectFit" class="item-icon" v-if="v.isNew" />
						<image src="/static/advance-icon.png" mode="aspectFit" class="item-icon" v-if="v.isAdvance" />
					</view>
			...
			<view class="items-box" v-show="!isNew">
				<view class="series">
					...
					>
						<image class="series-box-image" mode="aspectFill" src="/static/all-icon.png" />
					...
					<view class="series-box" @click="isShowBottom=true">
